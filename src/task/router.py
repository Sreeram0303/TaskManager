...
async def create_task(body: TaskSchema, background_tasks: BackgroundTasks,
    ...
    new_task = await controller.create_task(body, db, current_user.id)
...
async def get_all_tasks(db:DbSession,current_user:CurrentUser):
    return await controller.get_tasks(db,current_user.id)
...
async def modify_task(task_id:int,body:TaskUpdateSchema,
                ...
                db:DbSession,
                current_user:CurrentUser):
    modified_task =  await controller.modify_task(task_id,body,db,current_user.id)
...
async def update_task(task_id:int,
                ...
                db:DbSession,
                current_user:CurrentUser):
    updated_task = await controller.update_task(task_id,body,db,current_user.id)
...
async def delete_task(task_id:int,
    ...
    await controller.delete_task(task_id,db,current_user.id)
